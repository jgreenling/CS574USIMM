...
request_t * insert_read(long long int physical_address, long long int arrival_time, int thread_id, int instruction_id, long long int instruction_pc)
{
	...
	/*printf("new READ node: %Lx ",new_node->dram_addr.actual_address);
	printf("Bank: %d ",new_node->dram_addr.bank);
	printf("Rank: %d ",new_node->dram_addr.rank);
	printf("arrival_time: %d\n",new_node->arrival_time);
	printf("Oper Type: %d ",new_node->operation_type);
	printf("instruction_id: %d ",new_node->instruction_id);
	printf("next: %d \n",new_node->next);*/
...
void print_stats(int channel)
{
	...
	printf("Row buffer hit for reads:\n");
	for(int i = 0; i < NUM_CHANNELS; i++)
	{
		printf("\tChannel: %d\n", i);
		for(int j = 0; j < NUM_RANKS; j++)
	...
	printf("Their computed total reads: %lld\n\n", read_cmds);

	long long int totalWrites = 0;
	printf("Row buffer hit for writes:\n");
	for(int i = 0; i < NUM_CHANNELS; i++)
	{
		printf("\tChannel: %d\n", i);
		for(int j = 0; j < NUM_RANKS; j++)
	...
	printf("Their computed total writes: %lld\n\n", write_cmds);
}
