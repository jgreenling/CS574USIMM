...
void schedule(int channel)
{
	...
	printf("drain_writes %d", drain_writes[channel]);
	printf("write_queue_length %d", write_queue_length[channel]);
	printf("read_queue_length %d\n", read_queue_length[channel]);
	...
	if(drain_writes[channel])
	{
		...
		{
			printf("Address queue %Lx \n", wr_ptr->dram_addr.actual_address);
			printf("Command issuable %d \n", wr_ptr->command_issuable);
			if(wr_ptr->command_issuable)
			{
				issue_request_command(wr_ptr);
				printf("request_served %d \n", wr_ptr->request_served);
				break;
			}
		}
		return;
